...
class Augmenter(object):
    ...
    def augment(self, tokens, labels, op='del'):
        ...
        if 'del' in op:
            ...
            # Delete one random token of the sentence pairs
            # Not one token pro sentence
            ...
            # Shuffle tokens between pos1 and pos2
            ...
            # length is a randomly chosen token length among all tokens
            ...
            # Delete characters that are not number in randomly chosen token
            def drop_sym(token):
            ...
            # Drop a randomly chosen token that appears in both sentences of the pair
            left_token = set([])
            right_token = set([])
            left = True
            for token, label in zip(tokens, labels):
            ...
            targets = random.choices(list(same), k=1) if len(same) > 0 else []
            ...
            # Iterates over all tokens in sentence pair and drop them randomly with prob=0.25
            new_tokens, new_labels = [], []
            for token, label in zip(tokens, labels):
            ...
            # Deletes randomly one column name "COL ... VAL"
            ...
            # Drop randomly one column name + column val "COL ... VAL ..."
            col_starts = [i for i in range(len(tokens)) if tokens[i] == 'COL']
            col_ends = [0] * len(col_starts)
            col_lens = [0] * len(col_starts)
            for i, pos in enumerate(col_starts):
...
if __name__ == '__main__':
    ag = Augmenter()
    text = 'COL content VAL vldb conference papers 2020-01-01 COL year VAL 2020 ' \
           '[SEP] COL content VAL sigmod conference 2010 papers 2019-12-31 COL year VAL 2019'
    for op in ['del',
               'drop_col',
               'append_col',
               'drop_token',
